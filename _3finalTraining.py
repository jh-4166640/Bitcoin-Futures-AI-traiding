import os
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'    #쓸데없는 문장 지우기
import numpy as np
import pandas as pd
...
def generate_training_data(data, long_model, short_model, entry_prices, position_size):
    ...
    """
    long_predictions = long_model.predict(data)  # 롱 모델 예측 수행
    short_predictions = short_model.predict(data)  # 숏 모델 예측 수행
...
if __name__ == "__main__":
    # 데이터 로드
    data = pd.read_csv("btc_futures_data.csv")  # 2021-01-01부터 1분 간격 데이터
